...
# for colors_in, use either Tribe_colors or Subfamily_colors & relationsshpi, list either Tribe or `Sub-family`
box_plot_tribe_plot <- function(df_in, colors_in, relationship){
  if(length(unique(df_in$`Botanical name`)) > 1){
    hold_plot <- ggplot(df_in, aes(x = variable, y = value, fill = relationship)) +
      geom_boxplot(alpha = 0.5, outlier.alpha = 0) +
      geom_point(aes(x = variable), shape = 21, size = 2,  
                 position = position_jitterdodge(jitter.width = 0.8, jitter.height = 0.4, dodge.width = 0.9)) +
      #geom_jitter(size = 2, alpha = 0.8, width = 0.2, colour = "black") +
      my_ggplot_theme +
      ylab("Max RLUs") +
      xlab("") +
      scale_y_log10(limits = c(1, max(df_in$value))) +
      ...
      #scale_y_log10(breaks = c(1,200, 500, 1000, 5000, 20000, 50000, 100000), labels = c(0,200, 500, 1000, 5000, 20000, 50000, 100000),limits = c(1,max(df_in$value))) +
      theme(legend.position = "none") +
      scale_fill_manual("", values = colors_in) +
      scale_colour_manual("", values = colors_in) +
      labs(subtitle = c(as.character(unique(relationship))))
  }
  if(length(unique(df_in$`Botanical name`)) < 2){
    ...
    hold_plot <- ggplot(df_in, aes(x = variable, y = value, fill = relationship)) +
      ...
      #scale_y_continuous(breaks = c(0,200, 500, 1000, 5000, 20000, 50000, 100000), labels = c(0,200, 500, 1000, 5000, 20000, 50000, 100000), limits = c(1,100000)) +
      #scale_y_log10(breaks = c(0,200, 500, 1000, 5000, 20000, 50000, 100000), labels = c(0,200, 500, 1000, 5000, 20000, 50000, 100000),limits = c(1,max(df_in$value))) +
      theme(legend.position = "none") +
      scale_fill_manual("", values = colors_in) +
      scale_colour_manual("", values = colors_in) +
      labs(subtitle = c(as.character(unique(relationship))))
...
Toddalioideae_box <- box_plot_tribe_plot(Toddalioideae_melt, Subfamily_colors, Toddalioideae_melt$`Sub-family`)

# Rutoideae Subfamily
#Rutoideae_melt <- reshape2::melt(Rutoideae)
#Rutoideae_box <- box_plot_tribe_plot(Rutoideae_melt, Subfamily_colors, Rutoideae_melt$`Sub-family`)

# Aurantioideae Subfamily
Aurantioideae_melt <- reshape2::melt(Aurantioideae)
Aurantioideae_box <- box_plot_tribe_plot(Aurantioideae_melt, Subfamily_colors, Aurantioideae_melt$`Sub-family`)


# Break up Aurantioideae Subfamily into Tribes
Balsamocitrinae_melt <- reshape2::melt(Balsamocitrinae)
Balsamocitrinae_box <- box_plot_tribe_plot(Balsamocitrinae_melt, Tribe_colors, Balsamocitrinae_melt$Tribe)

Citrinae_melt <- reshape2::melt(Citrinae)
Citrinae_box <- box_plot_tribe_plot(Citrinae_melt, Tribe_colors, Citrinae_melt$Tribe)

Clauseninae_melt <- reshape2::melt(Clauseninae)
Clauseninae_box <- box_plot_tribe_plot(Clauseninae_melt, Tribe_colors, Clauseninae_melt$Tribe)

Merrilliinae_melt <- reshape2::melt(Merrilliinae)
Merrilliinae_box <- box_plot_tribe_plot(Merrilliinae_melt, Tribe_colors, Merrilliinae_melt$Tribe)

Micromelinae_melt <- reshape2::melt(Micromelinae)
Micromelinae_box <- box_plot_tribe_plot(Micromelinae_melt, Tribe_colors, Micromelinae_melt$Tribe)

Triphasiinae_melt <- reshape2::melt(Triphasiinae)
Triphasiinae_box <-box_plot_tribe_plot(Triphasiinae_melt, Tribe_colors, Triphasiinae_melt$Tribe)


#png("Comparison_of_Max_RLUs_across_Tribes.png", height = 8, width = 3.5, units = "in", res = 800)


ggpubr::ggarrange(NULL, NULL, Balsamocitrinae_box, 
                  Toddalioideae_box, NULL, Citrinae_box, 
                  NULL, NULL, Clauseninae_box, 
                  Aurantioideae_box, NULL, Merrilliinae_box, 
                  NULL, NULL, Micromelinae_box, 
                  NULL, NULL, Triphasiinae_box,
                  ncol = 3, nrow = 6, legend = "none")
...
png("Degress_of_variation_Comparison_of_Max_RLUs_across_Subfamily_as_facets.png", height = 6, width = 3, units = "in", res = 800)
...
plot_by_PAMP <- function(df_in){
  ggplot(df_in, aes(x = Tribe, y = value, fill = Tribe)) +
    ...
    xlab("") +
    #scale_y_continuous(limits = c(-1,100000), breaks =c(10,1000,10000,100000), labels = c(10,1000,10000,100000)) +
    scale_y_log10(limits = c(-1,100000), breaks =c(10,1000,10000,100000), labels = c(10,1000,10000,100000)) +
    scale_fill_manual("Tribe", values = Tribe_colors) 
}
